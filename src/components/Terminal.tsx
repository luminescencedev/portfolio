...
const Terminal = ({ children, className }: TerminalProps) => {
  ...
  useEffect(() => {
    ...
    const interactInstance = interact(terminal).draggable({
      listeners: {
        move(event) {
  ...
  return (
    <>
      ...
      <div
        ...
        className={cn(
          "card",
          "z-10 h-[calc(100vh-7rem)] w-[calc(100vw-7rem)] absolute flex items-center justify-center rounded-xl cursor-move",
          "before:content-[''] before:absolute before:inset-0 before:z-0 before:overflow-hidden before:rounded-[28px]",
          className
        )}
        style={{
      ...
      >
        {children || (
          <h1 className="text-6xl font-bold text-zinc-900 z-10 relative">
            Hello, World!
          </h1>
        )}
      </div>
